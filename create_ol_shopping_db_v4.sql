...
CREATE TABLE IF NOT EXISTS `ol_shopping`.`order_header` (
  ...
  INDEX `fk_order_header_shipping_address1_idx` (`shipping_address_no` ASC) VISIBLE,
  INDEX `fk_order_header_payment_method1_idx` (`payment_method_no` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_order_header_shipping_address1`
    FOREIGN KEY (`shipping_address_no`)
    REFERENCES `ol_shopping`.`shipping_address` (`address_no`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_order_header_payment_method1`
    FOREIGN KEY (`payment_method_no`)
